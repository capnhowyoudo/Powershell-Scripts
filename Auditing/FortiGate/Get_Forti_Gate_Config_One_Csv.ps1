...
.EXAMPLE
    ...
    .\Get_Forti_Gate_Config_One_Csv.ps1 -FortiGateHost "192.168.1.1" -ApiToken "YourApiTokenHere"

.EXAMPLE
    # Self-signed cert (most common for on-prem FortiGates)
    .\Get_Forti_Gate_Config_One_Csv.ps1 -FortiGateHost "192.168.1.1" -ApiToken "YourApiTokenHere" -SkipCertificateCheck

.EXAMPLE
    # Non-standard management port
    .\Get_Forti_Gate_Config_One_Csv.ps1 -FortiGateHost "192.168.1.1" -ApiToken "YourApiTokenHere" -Port 8443 -SkipCertificateCheck

.EXAMPLE
    # Full example with all common options
    .\Get_Forti_Gate_Config_One_Csv.ps1 -FortiGateHost "FW_IP" -ApiToken "YourApiTokenHere" -Port 8443 -SkipCertificateCheck

.EXAMPLE
    # Username and password instead of API token
    .\Get_Forti_Gate_Config_One_Csv.ps1 -FortiGateHost "192.168.1.1" -Credential (Get-Credential) -SkipCertificateCheck

.EXAMPLE
    # Query a specific VDOM
    .\Get_Forti_Gate_Config_One_Csv.ps1 -FortiGateHost "192.168.1.1" -ApiToken "YourApiTokenHere" -Vdom "CORP" -SkipCertificateCheck
...
Write-Host "  Exporting data..." -ForegroundColor Magenta
Write-Host "============================================================" -ForegroundColor Magenta

# ---------------------------------------------------------------------------
# Helper: flatten nested arrays in objects to semicolon-separated strings
# ---------------------------------------------------------------------------
...
$tabs = [ordered]@{
    ...
    'DNS_Settings'      = if ($dnsResult.Settings) { Flatten-Data -Data @($dnsResult.Settings) } else { @() }
    'DNS_Servers'       = Flatten-Data -Data $dnsResult.DnsServers
    'DNS_Zones'         = Flatten-Data -Data $dnsResult.Zones
    'DNS_Static'        = Flatten-Data -Data $dnsResult.StaticEntries
    'DHCP_Servers'      = Flatten-Data -Data $dhcpResult.Servers
    'DHCP_Ranges'       = Flatten-Data -Data $dhcpResult.Ranges
    'DHCP_Reservations' = Flatten-Data -Data $dhcpResult.Reservations
    'DHCP_Options'      = Flatten-Data -Data $dhcpResult.Options
    'Users_Local'       = Flatten-Data -Data $userResult.LocalUsers
    'Users_LDAP'        = Flatten-Data -Data $userResult.LdapServers
...
# ---------------------------------------------------------------------------
# Try Excel export via ImportExcel module
# ---------------------------------------------------------------------------
...
if (Get-Module -ListAvailable -Name ImportExcel) {
    ...
    try {
        # Force NuGet provider silently - suppresses the interactive Y/N prompt
    ...
    try {
        ...
        Write-Warning '  Falling back to TSV export. To enable Excel export run:'
        Write-Warning '  Install-PackageProvider -Name NuGet -Force'
        Write-Warning '  Install-Module -Name ImportExcel -Scope CurrentUser -Force'
    }
}
...
if ($hasImportExcel) {
    ...
    foreach ($tabName in $tabs.Keys) {
        ...
        if ($count -eq 0) {
            # Write a placeholder row so the tab still exists and is visible
            $placeholder = [PSCustomObject]@{ Info = '(no data)' }
            $placeholder | Export-Excel -Path $xlsxPath -WorksheetName $tabName `
                           -AutoSize -FreezeTopRow -BoldTopRow -Append
            Write-Host ('  [EMPTY] ' + $tabName) -ForegroundColor DarkGray
        } else {
            $data | Export-Excel -Path $xlsxPath -WorksheetName $tabName `
                    -AutoSize -FreezeTopRow -BoldTopRow -Append
            Write-Host ('  [OK]    ' + $tabName + ' (' + $count + ' row(s))') -ForegroundColor Green
        }
    }

    Write-Host ("`nExcel workbook saved to: " + $xlsxPath) -ForegroundColor Cyan

} else {
    # ---------------------------------------------------------------------------
    # Fallback: single TSV file with section headers between tables
    # ---------------------------------------------------------------------------
    $tsvLines = @()
    $tsvLines += ('FortiGate Configuration Export')
    $tsvLines += ('Host: ' + $FortiGateHost + '    VDOM: ' + $Vdom + '    Exported: ' + (Get-Date))
    $tsvLines += ''
    ...
    $tsvLines | Set-Content -Path $tsvPath -Encoding UTF8
    Write-Host ("`nTSV file saved to: " + $tsvPath) -ForegroundColor Cyan
    Write-Host '  Open in Excel: Data > From Text/CSV, set delimiter to Tab.' -ForegroundColor Yellow
}
